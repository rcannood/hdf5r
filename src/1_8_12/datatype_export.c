...
SEXP init_dtypes(void) {
  ...
  h5_datatype[DT_H5F_info_helper_t] = create_DT_H5F_info_helper_t();
  h5_datatype[DT_H5F_info_t] = create_DT_H5F_info_t();
...
SEXP show_all_types_data_frame(void) {
  ...
  SET_STRING_ELT(R_categ, DT_H5F_info_helper_t, mkChar("struct"));
  SET_STRING_ELT(R_name, DT_H5F_info_helper_t, mkChar("H5F_info_helper_t"));
  ((long long *) REAL(R_type))[DT_H5F_info_helper_t] = h5_datatype[DT_H5F_info_helper_t];

  SET_STRING_ELT(R_categ, DT_H5F_info_t, mkChar("struct"));
  SET_STRING_ELT(R_name, DT_H5F_info_t, mkChar("H5F_info_t"));
  ((long long *) REAL(R_type))[DT_H5F_info_t] = h5_datatype[DT_H5F_info_t];
...
hid_t create_DT_H5D_chunk_index_t(void) {
  ...
  hid_t dtype_id = H5Tenum_create(base_type);
  myenum = H5D_CHUNK_BTREE;
  H5Tenum_insert(dtype_id, "H5D_CHUNK_BTREE", &myenum);
  return(dtype_id);
}
...
hid_t create_DT_H5D_mpio_no_collective_cause_t(void) {
  ...
  myenum = H5D_MPIO_SET_MPIPOSIX;
  H5Tenum_insert(dtype_id, "H5D_MPIO_SET_MPIPOSIX", &myenum);
  myenum = H5D_MPIO_NOT_SIMPLE_OR_SCALAR_DATASPACES;
  H5Tenum_insert(dtype_id, "H5D_MPIO_NOT_SIMPLE_OR_SCALAR_DATASPACES", &myenum);
  myenum = H5D_MPIO_POINT_SELECTIONS;
  H5Tenum_insert(dtype_id, "H5D_MPIO_POINT_SELECTIONS", &myenum);
...
hid_t create_DT_H5AC_cache_config_t(void) {
  ...
  H5Tinsert(dtype_id, "dirty_bytes_threshold", HOFFSET(H5AC_cache_config_t, dirty_bytes_threshold), h5_datatype[DT_int]);
...
hid_t create_DT_H5F_info_helper_t(void) {
  hid_t dtype_id = H5Tcreate(H5T_COMPOUND, sizeof(H5F_info_helper_t));
  H5Tinsert(dtype_id, "hdr_size", HOFFSET(H5F_info_helper_t, hdr_size), h5_datatype[DT_hsize_t]);
  H5Tinsert(dtype_id, "msgs_info", HOFFSET(H5F_info_helper_t, msgs_info), h5_datatype[DT_H5_ih_info_t]);
  return(dtype_id);
}

/* typedef struct H5F_info_t { hsize_t super_ext_size; H5F_info_helper_t sohm} H5F_info_t; */
hid_t create_DT_H5F_info_t(void) {
  hid_t dtype_id = H5Tcreate(H5T_COMPOUND, sizeof(H5F_info_t));
  H5Tinsert(dtype_id, "super_ext_size", HOFFSET(H5F_info_t, super_ext_size), h5_datatype[DT_hsize_t]);
  H5Tinsert(dtype_id, "sohm", HOFFSET(H5F_info_t, sohm), h5_datatype[DT_H5F_info_helper_t]);
  return(dtype_id);
}
